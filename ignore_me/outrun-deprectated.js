...
(function () {
	...
	const HUD_ATTR = BG_MAGENTA | LIGHTCYAN;
	const HUD_TITLE_ATTR = BG_MAGENTA | YELLOW;
	const HUD_INFO_ATTR = BG_MAGENTA | WHITE;
	const SKY_ATTRS = [
		BG_MAGENTA | LIGHTMAGENTA,
		BG_MAGENTA | LIGHTRED,
		BG_RED | YELLOW,
		BG_RED | WHITE
	];
	...
	const LANE_ATTR = BG_BLACK | YELLOW;
	const CAR_ATTR_TOP = BG_CYAN | WHITE;
	const CAR_ATTR_BOTTOM = BG_CYAN | LIGHTCYAN;
	const CAR_ATTR_SIDE = BG_CYAN | LIGHTCYAN;
	const CAR_ATTR_WINDOW = BG_BLUE | LIGHTCYAN;
	const TAIL_LIGHT_ATTR = BG_CYAN | LIGHTRED;
	const CAR_WHEEL_ATTR = BG_BLACK | DARKGRAY;
	const TREE_ATTR = BG_GREEN | LIGHTCYAN;
	const SIGN_ATTR = BG_BROWN | YELLOW;
	...
	const SUN_SHAPE = [
		{ dx: 0, dy: 0, ch: ascii(15), attr: BG_BROWN | YELLOW },
		{ dx: -1, dy: 0, ch: ascii(248), attr: BG_BROWN | YELLOW },
		{ dx: 1, dy: 0, ch: ascii(248), attr: BG_BROWN | YELLOW },
		{ dx: 0, dy: -1, ch: ascii(248), attr: BG_BROWN | YELLOW },
		{ dx: 0, dy: 1, ch: ascii(248), attr: BG_BROWN | YELLOW }
	];

	// Convert CGA attribute byte to Ctrl-A color code sequence
	function attrToCtrlA(attr) {
		var str = '';

		// Background color (bits 4-6, shifted right by 4)
		var bg = (attr >> 4) & 0x07;
		str += '\x01' + bg.toString();

		// Foreground color mapping (bits 0-2)
		var fgColors = ['k', 'b', 'g', 'c', 'r', 'm', 'y', 'w'];
		var fg = attr & 0x07;
		str += '\x01' + fgColors[fg];

		// High intensity (bit 3)
		if (attr & HIGH) {
			str += '\x01h';
		}
...
var lastHudScore = -1;
	...
	function writeCenteredHud(row, attr, text, key) {
		...
		console.print(attrToCtrlA(attr) + output);
		hudCache[key] = text;
	}
	...
	function fillSpan(row, start, end, attr, ch) {
		...
		console.print(attrToCtrlA(attr) + cachedFill(ch, len));
	}
	...
	function drawGlyph(row, col, attr, ch) {
		...
		console.print(attrToCtrlA(attr) + ch);
	}
